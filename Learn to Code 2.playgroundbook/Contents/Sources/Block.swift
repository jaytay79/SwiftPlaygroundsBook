...
public final class Block: Item, LocationConstructible, NodeConstructible {
    ...
    static let wallTextures: [URL] = {
        let textureURLs = Bundle.main.urls(forResourcesWithExtension: "png", subdirectory: Asset.Directory.textures.path) ?? []
        return textureURLs.filter {
            $0.lastPathComponent.hasPrefix("zon_block_a_rock_DIFF_")
        }
    }()
    
    static let topTextures: [URL] = {
    ...
    lazy var texturesForDirection: [Direction: URL] = {
        ...
        for dir in Direction.all {
            let textureLevel: Int = Block.levelFromName(for: self.scnNode) % Block.patternCount
            
            let suffix = "u\(textureLevel)_v\(dir.index + 1).png"
            let suffixURLs = Block.wallTextures.filter { $0.lastPathComponent.hasSuffix(suffix) }
            guard let url = suffixURLs.first else { continue }
