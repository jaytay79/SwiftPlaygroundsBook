...
In the previous puzzle, your turned right only once, so using three left turns wasn't a problem. But what if you need to turn right more than once? It would be more efficient to put all those left turns into a ``turnRight()`` command that you run multiple times.
