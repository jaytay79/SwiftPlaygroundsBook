...
public class GridWorld {
    ...
    /// to the new position, and rotates the item if necessary. 
    func addCommandToReplace(_ item: Item, newPosition: SCNVector3, newDirection: Direction?) {
...
extension GridWorld: Performer {
    ...
    func perform(_ action: Action) -> Bool {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + extendedDuration) {
            self.commandQueue.performerFinished(self)
        }
...
extension GridWorld {
    ...
    public func place(_ item: Item, facing: Direction? = nil, at coordinate: Coordinate) -> Item {
        ...
        if item.isInWorld && isAnimated {
            addCommandToReplace(item, newPosition: newPosition, newDirection: facing)
        }
