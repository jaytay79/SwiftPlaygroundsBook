...
public class Actor: Item, NodeConstructible {
    ...
    static var commandSpeed: Float = WorldConfiguration.Actor.possibleSpeeds[0]
    ...
    public weak var world: GridWorld? {
        didSet {
            // Ensure the actor's `commandDriver` is correctly set when adding
            // the Actor to the world.
            commandDriver = world?.commandQueue
    ...
    weak var commandDriver: PerformerDelegate? = nil
...
extension Actor {
    ...
    func perform(_ action: Action) -> Bool {
        // Clear the `currentAction` in case a valid event does not exist.
        currentAction = nil
        if isRunning {
            cancel(action)
        }
        ...
        for component in components {
            let componentIsRunning = component.perform(event: event, variation: index)
            if componentIsRunning {
                runningComponents.append(component)
            }
        }
...
extension Actor: PerformerDelegate {
    ...
    func performerFinished(_ performer: Performer) {
        ...
        if isRunning == false {
            commandDriver?.performerFinished(self)
        }
    }
}
