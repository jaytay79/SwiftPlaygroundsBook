...
public final class CommandQueue {
    ...
    func append(_ command: Command, applyingState: Bool) {
        ...
        commandQueue(self, added: command)
    ...
    */
    ...
    func runCommand(atIndex index: Int) {
        ...
        commandQueue(self, willPerform: command)
        
        command.perform()
        
        // If the command was not capable of running, continue through the queue. 
        if !command.isRunning && runMode == .continuous {
            runNextCommand()
        }
    }
...
/**
 Having CommandQueue conform to its own delegate ensures that both the `delegate`
 and `overflowDelegate` get called for every interaction.
 
 This also formalizes the contract if any other methods were to be added to the 
 protocol, that they would need to be updated here.
 */
extension CommandQueue: CommandQueueDelegate {
    func commandQueue(_ queue: CommandQueue, added command: Command) {
        guard reportsAddedCommands else { return }
        delegate?.commandQueue(queue, added: command)
        overflowDelegate?.commandQueue(queue, added: command)
    }
    
    func commandQueue(_ queue: CommandQueue, willPerform command: Command) {
        delegate?.commandQueue(queue, willPerform: command)
        overflowDelegate?.commandQueue(queue, willPerform: command)
    }
    
    func commandQueue(_ queue: CommandQueue, didPerform command: Command) {
        delegate?.commandQueue(queue, didPerform: command)
        overflowDelegate?.commandQueue(queue, didPerform: command)
    }
}

// MARK: PerformerDelegate

extension CommandQueue: PerformerDelegate {
    
    /**
     NOTE: All commands are eventually routed through this
     delegate call to drive the `commandQueue`.
     
     Actor commands are wired up through the actor's `commandDelegate`.
     */
    func performerFinished(_ performer: Performer) {
        precondition(Thread.isMainThread, "The performer must be called back on the main thread.")
        
        // Note: This is only valid when running a single command at a time.
        guard let command = currentCommand, command.performer === performer else {
            #if DEBUG
            assertionFailure("\(performer) finished for the wrong command: \(currentCommand)")
            #endif
            return
        }
        
        // Advance the index to indicate a command has been completed.
        currentIndex = commands.index(after: currentIndex)
        commandQueue(self, didPerform: command)
